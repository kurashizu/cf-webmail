<script lang="ts">
	...
	function openUser(user: any) { selected = { ...user }; modal = null; message = null; }
	...
	async function userAction(action: string, extra: Record<string, unknown> = {}) {
		if (!selected) return;
		busy = selected.id;
		busyLabel = action === 'update' ? 'Saving changes…' : action === 'reset_password' ? 'Resetting password…' : action === 'revoke_sessions' ? 'Signing out user…' : action === 'disable' ? 'Disabling mailbox…' : 'Enabling mailbox…';
		message = null;
		try {
			...
			users = [...users];
			showSuccess(action === 'reset_password' ? 'Password reset. Existing sessions were signed out.' : action === 'revoke_sessions' ? 'All sessions revoked.' : 'Account updated.');
	...
	async function copy(value: string, key: string) { try { await navigator.clipboard.writeText(value); copied = key; setTimeout(() => copied = '', 1600); } catch { message = { kind: 'error', text: tt('admin.users.copyError') }; } }
...
<svelte:head><title>{tt('admin.users.title')} · {tt('common.brandName')}</title></svelte:head>

<section class="page">
	<header class="page-head"><div><p class="eyebrow">{tt('common.brandName')}</p><h1>{tt('admin.users.title')}</h1><p class="subtitle">{tt('admin.users.subtitle')}</p></div><a class="btn btn-ghost" href="/settings">{tt('common.back')}</a></header>
	<div class="stats">
		<div class="stat card"><span>{tt('admin.users.stats.users')}</span><strong>{stats.users}</strong></div><div class="stat card"><span>{tt('admin.users.stats.active')}</span><strong>{stats.active}</strong></div><div class="stat card"><span>{tt('admin.users.stats.unread')}</span><strong>{stats.unread}</strong></div><div class="stat card"><span>{tt('admin.users.stats.invites')}</span><strong>{stats.invites}</strong></div>
	</div>
	{#if message}<div class="notice {message.kind}" role="status">{message.text}<button aria-label={tt('common.dismiss')} onclick={() => message = null}>×</button></div>{/if}
	<div class="control card">
		<div class="tabs"><button class:active={tab === 'users'} onclick={() => switchTab('users')}>{tt('admin.users.tabUsers')} <span>{users.length}</span></button><button class:active={tab === 'invites'} onclick={() => switchTab('invites')}>{tt('admin.users.tabInvites')} <span>{stats.invites}</span></button></div>
		<div class="tools"><label class="search"><svg viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="1.7"/><path d="m16.5 16.5 4 4" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg><input bind:value={query} placeholder={tab === 'users' ? tt('admin.users.searchUsers') : tt('admin.users.searchInvites')} /></label>
		<select bind:value={status}>{#if tab === 'users'}<option value="all">{tt('admin.users.filterAll')}</option><option value="active">{tt('admin.users.filterActive')}</option><option value="disabled">{tt('admin.users.filterDisabled')}</option><option value="admin">{tt('admin.users.filterAdmin')}</option><option value="user">{tt('admin.users.filterMember')}</option>{:else}<option value="all">{tt('admin.users.filterAll')}</option><option value="active">{tt('admin.users.filterActive')}</option><option value="used">{tt('admin.users.filterUsed')}</option><option value="expired">{tt('admin.users.filterExpired')}</option>{/if}</select>
		{#if tab === 'invites'}<button class="btn btn-primary" onclick={() => modal = 'invite'}><svg viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>{tt('admin.users.actionInviting')}</button>{/if}</div>
	</div>

	{#if generated}<div class="generated card"><div><span>{tt('admin.users.invite.codeHeading')}</span><code>{generated.code}</code><small>{generated.local_part ? tt('admin.users.reservedFor', { local: generated.local_part, domain: 'krsz.in' }) : tt('admin.users.recipientChooses')}</small></div><div><button class="btn btn-primary" onclick={() => copy(generated.code, 'code')}>{copied === 'code' ? tt('common.copied') : tt('admin.users.invite.copyCode')}</button><button class="btn btn-ghost" onclick={() => copy(`${location.origin}/register?invite=${generated.code}`, 'link')}>{copied === 'link' ? tt('common.copied') : tt('admin.users.invite.copyLink')}</button><button class="icon-btn" aria-label={tt('common.dismiss')} onclick={() => generated = null}>×</button></div></div>{/if}

	{#if loading}<div class="loading">{tt('common.loading')}</div>
	{:else if tab === 'users'}
		<div class="table-card card"><div class="table-wrap"><table><thead><tr><th>{tt('admin.users.colUser')}</th><th>{tt('admin.users.colRole')}</th><th>{tt('admin.users.colMailbox')}</th><th>{tt('admin.users.colStorage')}</th><th>{tt('admin.users.colLastMail')}</th><th>{tt('admin.users.colStatus')}</th><th></th></tr></thead><tbody>{#each filteredUsers as user (user.id)}<tr class:disabled={user.disabled}><td><strong>{user.display_name || user.local_part}</strong><span>{user.email}</span></td><td><span class="badge role">{user.role === 'admin' ? tt('admin.users.roleAdmin') : tt('admin.users.roleUser')}</span></td><td>{tt('admin.users.messagesBadge', { count: user.message_count || 0 })}<br/><small>{tt('admin.users.unreadBadge', { count: user.unread_count || 0 })}</small></td><td>{formatBytes(Number(user.storage_bytes || 0))}</td><td>{user.last_message_at ? new Date(user.last_message_at).toLocaleDateString() : tt('admin.users.lastMailNever')}</td><td><span class="badge" class:off={user.disabled}>{user.disabled ? tt('admin.users.statusDisabled') : tt('admin.users.statusActive')}</span></td><td><button class="btn btn-ghost" onclick={() => openUser(user)}>{user.id === data.user.accountId ? tt('admin.users.openUserSelf') : tt('admin.users.openUser')}</button></td></tr>{/each}</tbody></table></div>{#if !filteredUsers.length}<div class="empty">{tt('admin.users.emptyUsers')}</div>{/if}</div>
	{:else}
		<div class="table-card card"><div class="table-wrap"><table><thead><tr><th>{tt('admin.users.colInvitation')}</th><th>{tt('admin.users.colCreated')}</th><th>{tt('admin.users.colExpires')}</th><th>{tt('admin.users.colUsedBy')}</th><th>{tt('admin.users.colStatus')}</th><th></th></tr></thead><tbody>{#each filteredInvites as invite (invite.code_hash)}{@const inviteStatus = invite.consumed_at ? 'used' : invite.expires_at && invite.expires_at < Date.now() ? 'expired' : 'active'}<tr><td><strong>{invite.local_part ? `${invite.local_part}@krsz.in` : tt('admin.users.anyAddress')}</strong><span>{invite.notes || tt('admin.users.notesNone')}</span></td><td>{new Date(invite.created_at).toLocaleString()}<br/><small>{tt('admin.users.createdBy', { email: invite.created_by_email || tt('admin.users.createdByAdmin') })}</small></td><td>{invite.expires_at ? new Date(invite.expires_at).toLocaleString() : tt('admin.users.lastMailNever')}</td><td>{invite.consumed_by_email || '—'}</td><td><span class="badge {inviteStatus}">{inviteStatus === 'used' ? tt('admin.users.statusUsed') : inviteStatus === 'expired' ? tt('admin.users.statusExpired') : tt('admin.users.statusActive')}</span></td><td>{#if !invite.consumed_at}<button class="btn danger" disabled={busy === invite.code_hash} onclick={() => deleteInvite(invite)}>{inviteStatus === 'active' ? tt('admin.users.revokeButton') : tt('admin.users.deleteInviteButton')}</button>{:else}<span class="muted">{tt('admin.users.statusRetained')}</span>{/if}</td></tr>{/each}</tbody></table></div>{#if !filteredInvites.length}<div class="empty">{tt('admin.users.emptyInvites')}</div>{/if}</div>
	{/if}
</section>
...
{#if selected}<div class="backdrop" role="presentation" onclick={(e) => e.target === e.currentTarget && closePanel()}><aside class="panel" aria-label="User details"><header><div><span>Mailbox account</span><h2>{selected.email}</h2></div><button class="icon-btn" onclick={closePanel} aria-label="Close">×</button></header><div class="panel-body">
	...
	<div class="metrics"><div><span>Messages</span><strong>{selected.message_count || 0}</strong></div><div><span>Unread</span><strong>{selected.unread_count || 0}</strong></div><div><span>Storage</span><strong>{formatBytes(Number(selected.storage_bytes || 0))}</strong></div><div><span>Session</span><strong>{selected.has_session ? 'Signed in' : 'None'}</strong></div></div>
...
<style>
	...
	@media(max-width:760px){.page{padding:14px 10px calc(72px + 14px + env(safe-area-inset-bottom, 0px))}.page-head{align-items:flex-start;flex-direction:column}.stats{gap:7px}.stat{padding:12px}.stat strong{font-size:19px}.tools{flex-direction:column}.tools select,.tools .btn{width:100%}.generated>div:last-child{width:100%;flex-direction:column}.metrics{grid-template-columns:1fr 1fr}.panel-body{padding:13px}.action-row{align-items:flex-start;flex-direction:column}.action-row .btn{width:100%}}

	/* Keep the user drawer below the persistent mail header. */
	.backdrop {
		z-index: 45;
