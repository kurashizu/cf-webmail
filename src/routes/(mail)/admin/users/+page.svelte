...
<section class="page">
	...
	{:else if tab === 'users'}
		<div class="table-card card"><div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Mailbox</th><th>Storage</th><th>Last mail</th><th>Status</th><th></th></tr></thead><tbody>{#each filteredUsers as user (user.id)}<tr class:disabled={user.disabled}><td><strong>{user.display_name || user.local_part}</strong><span>{user.email}</span></td><td><span class="badge role">{user.role === 'admin' ? 'Admin' : 'Member'}</span></td><td>{user.message_count || 0} messages<br/><small>{user.unread_count || 0} unread</small></td><td>{formatBytes(Number(user.storage_bytes || 0))}</td><td>{user.last_message_at ? new Date(user.last_message_at).toLocaleDateString() : 'Never'}</td><td><span class="badge" class:off={user.disabled}>{user.disabled ? 'Disabled' : 'Active'}</span></td><td><button class="btn btn-ghost" onclick={() => openUser(user)}>{user.id === data.user.accountId ? 'View' : 'Manage'}</button></td></tr>{/each}</tbody></table></div>{#if !filteredUsers.length}<div class="empty">No matching users.</div>{/if}</div>
	{:else}
